...
//   // Workaround for Node.js DNS resolution ETIMEOUT on Windows:
//   // Use OS native dns.lookup to resolve the hostname before passing to nodemailer
//   let resolvedHost = HOST;
//   try {
//     const { address } = await dnsPromises.lookup(HOST);
//     if (address) {
//       resolvedHost = address;
//       console.log(`Resolved ${HOST} to ${address} via dns.lookup`);
//     }
//   } catch (err) {
...
export async function POST(request: Request) {
  ...
  const HOST = process.env.SMTP_HOST || 'server120.web-hosting.com'
  ...
  const transporter = nodemailer.createTransport({
    ...
    // Debug mode (verbose only outside production)
    ...
    greetingTimeout: 20_000,
    socketTimeout: 60_000,
    // Let the server pick the best auth method
    // authMethod intentionally omitted
    requireTLS: true,
    tls: {
      // Enforce trusted certs for deliverability
  ...
  const BATCH_SIZE = 5000              // smaller batches reduce spikes that trigger filters
  const CONCURRENCY = 500              // low parallelism lowers throttling and spam risk
  const BATCH_DELAY_MS = 10_000      // pause between batches
  const MAX_RETRIES = 3
  const BASE_BACKOFF_MS = 2_000
  ...
  const sendOneWithRetry = async (rcpt: string): Promise<Result> => {
    ...
    while (attempt <= MAX_RETRIES) {
      try {
        ...
        const html = formatHtmlContent(personalized, subject, rcpt)
        const text = toPlainText(personalized)
        const authDomain = (AUTH_USER.split('@')[1] || 'yourdomain.com').toLowerCase()
        const messageId = `<${Date.now()}.${Math.random().toString(36).substring(2)}@${authDomain}>`
        ...
        const mailOptions = {
          ...
          text,
          html,
          // Keep headers minimal to avoid bulk/promotional signals
          headers: {
            'Message-ID': messageId,
            'X-Auto-Response-Suppress': 'OOF, AutoReply'
          },
          // Ensure replies go to the visible sender
          replyTo: `"${displayName}" <${sender.email}>`,
          // Envelope must match your authenticated domain
          envelope: { 
            from: AUTH_USER, 
            to: rcpt 
          },
          // DKIM signing: align with the authenticated domain
          dkim: process.env.DKIM_PRIVATE_KEY ? {
